            ...
            .then(data => {
                ...
                if (data.success && data.authorized) {
                    ...
                    setTimeout(() => {
                        ...
                        // Begin polling the validation status endpoint
                        let validationAttempts = 0;
                        const maxValidationAttempts = 10;
                        
                        /*const checkValidation = () => {
                            validationAttempts++;
                            console.log(`Checking auth validation status (attempt ${validationAttempts}/${maxValidationAttempts})...`);
                            
                            fetch('/api/validate_auth')
                                .then(response => response.json())
                                .then(data => {
                                    if (data.error) {
                                        ...
                                        if (validationAttempts < maxValidationAttempts) {
                                            ...
                                            showToast('Warning', 'Authentication verification timed out. Try refreshing the page.', 'warning');
                                        }
                                    } 
                                    else if (data.logged_in && data.username) {
                                        // Auth is valid and complete
                                        showToast('Success', `Logged in as ${data.username}`, 'success');
                                        console.log('Login validation successful:', data);
                                        
                                        // Refresh the UI with the new authentication state
                                        refreshData({
                                            refreshLogin: true,
                                            refreshInventory: true
                                        });
                                    } 
                                    else if (data.validating) {
                                        // Still validating, check again in a second
                                        if (validationAttempts < maxValidationAttempts) {
                                            setTimeout(checkValidation, 1000);
                                        } else {
                                            showToast('Warning', 'Authentication validation is taking longer than expected. Try refreshing the page.', 'warning');
                                        }
                                    }
                                    else {
                                        // Not logged in or error
                                        console.warn('Login may not have completed properly:', data);
                                        if (validationAttempts < maxValidationAttempts) {
                                            setTimeout(checkValidation, 1000);
                                        } else {
                                            // Last attempt, force a refresh
                                            showToast('Info', 'Refreshing application data...', 'info');
                                            refreshData({
                                                refreshLogin: true,
                                                refreshInventory: true
                                            });
                        ...
                        // Start checking validation after a short delay
                        setTimeout(checkValidation, 1000);
                    }, 2000);
