...
class HeaderBar:
    ...
    def build(self, initial_tab: str, on_tab_change):
        ...
        ui.context.client.on_disconnect(lambda: self._client_labels.pop(client_id, None))
        ...
        with ui.header().classes('flex-col items-stretch p-0 gap-0'):
            ...
            with ui.tabs(value=initial_tab, on_change=on_tab_change).classes('w-full') as tabs:
                ui.tab('main',      label=_('gui', 'tabs', 'main'),      icon='home')
                ui.tab('inventory', label=_('gui', 'tabs', 'inventory'), icon='inventory')
