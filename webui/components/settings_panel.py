...
class SettingsPanel:
                ...
                with ui.row().classes('items-center gap-2 text-xs'):
                    ...
                    dark_switch = ui.switch(
                        ...
                        on_change=lambda e: (
                            ...
                            manager._toggle_dark_mode(e.value),
                            self._sync_others(self._dark_mode_switches, client_id, e.value),
                        ),
                    )
                ...
                with ui.row().classes('items-center gap-2 text-xs'):
                    ...
                    enable_badges_switch = ui.switch(
                        ...
                        on_change=lambda e: (
                            ...
                            self._sync_others(self._enable_badges_switches, client_id, e.value),
                        ),
                    )
                ...
                with ui.row().classes('items-center gap-2 text-xs'):
                    ...
                    available_drops_switch = ui.switch(
                        ...
                        on_change=lambda e: (
                            ...
                            self._sync_others(self._available_drops_switches, client_id, e.value),
                        ),
                    )
    ...
    def _build_priority_column(self, client_id: str) -> None:
        ...
        with ui.card().props('flat bordered').classes('q-pa-sm flex flex-col grow shrink basis-60 min-w-0'):
            ...
            with ui.row().classes('w-full gap-1 items-start min-h-[200px]'):
                priority_list = ui.list().props('dense bordered').classes(
                ...
                with ui.column().classes('gap-1'):
                    ui.button('⏫', on_click=lambda: self._priority_move('top')).props('flat').classes('text-xl p-0 min-h-0')
                    ui.button('⬆️', on_click=lambda: self._priority_move('up')).props('flat').classes('text-xl p-0 min-h-0')
                    ui.button('⬇️', on_click=lambda: self._priority_move('down')).props('flat').classes('text-xl p-0 min-h-0')
                    ui.button('⏬', on_click=lambda: self._priority_move('bottom')).props('flat').classes('text-xl p-0 min-h-0')
                    ui.button('❌', on_click=self._priority_delete).props('flat').classes('text-red-500 text-xl p-0 min-h-0')
    ...
    def _build_exclude_column(self, client_id: str) -> None:
        ...
        with ui.card().props('flat bordered').classes('q-pa-sm flex flex-col grow shrink basis-60 min-w-0'):
            ...
            with ui.row().classes('w-full gap-1 items-start min-h-[200px]'):
                exclude_list = ui.list().props('dense bordered').classes(
                    'flex-1 text-xs overflow-y-auto min-h-[200px]'
                )
    ...
    def _rebuild_priority_list(self) -> None:
        ...
        for client_id, priority_list in list(self._priority_lists.items()):
            ...
            if client_id == current_id:
                ...
                asyncio.get_event_loop().create_task(
                    self._async_rebuild_priority(Client.instances[client_id], priority_list)
                )
    ...
    def _rebuild_exclude_list(self) -> None:
        ...
        for client_id, exclude_list in list(self._exclude_lists.items()):
            ...
            if client_id == current_id:
                ...
                asyncio.get_event_loop().create_task(
                    self._async_rebuild_exclude(Client.instances[client_id], exclude_list)
                )
            ...
            if client_id == current_id:
                ...
                asyncio.get_event_loop().create_task(
                    self._async_refresh_inputs(
                        Client.instances[client_id], p_input, e_input, priority_opts, exclude_opts
                    )
                )
    ...
    async def _async_refresh_inputs(
        self, client, p_input: 'NiceSelect | None', e_input: 'NiceSelect | None',
        priority_opts: list, exclude_opts: list,
    ) -> None:
        with client:
            if p_input is not None:
                p_input.options = priority_opts
